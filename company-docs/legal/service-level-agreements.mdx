...
description: "FarmGPU SLA commitments, uptime guarantees, and service credits"
icon: "file-contract"
---
...
Downtime start and end timestamps are determined from **FarmGPU's monitoring and telemetry systems**, which serve as the authoritative record for all SLA calculations. Relevant monitoring data for a disputed period will be shared with Customer on request. If Customer presents evidence of unavailability during a period where FarmGPU's monitoring has a gap or was itself unavailable, FarmGPU will consider Customer's records in good faith when validating the claim.
...
## Capacity Availability Guarantee
...
## Hardware Replacement Service Level (Targets)
...
| --- | --- |
| Individual GPU failure | 3 business days |
| Complete node failure | 2 weeks |
| Network infrastructure failure | 48 hours |
| Storage device failure | 48 hours |
| Memory failure | 3 business days |
| Power supply failure | 48 hours |

Where on-site spare hardware is available for the affected component, FarmGPU will use commercially reasonable efforts to resolve faster than these targets. Where resolution requires an OEM RMA cycle, the target runs from FarmGPU's receipt of the replacement part, and FarmGPU will provide equivalent replacement capacity in the interim where available.
...
Target resolution times are subject to site access, safety constraints, and hardware availability. Initial response times are governed by the severity matrix below.

## Support Response and Severity Classification

FarmGPU provides 24×7 incident response for Reserved Capacity and Bare Metal Services against the following severity matrix:

| PRIORITY | DESCRIPTION | INITIAL RESPONSE | TARGET FIX OR WORKAROUND |
| --- | --- | --- | --- |
| P1 | Service or node down or inaccessible; no workaround | 15 minutes | 8 hours |
| P2 | Service operational with substantial errors or degraded capacity | 4 hours | 1 business day |
| P3 | Errors impairing full benefit of the service; workaround available | 8 hours | 2 business days |
| P4 | Minor errors, cosmetic issues, general questions | 1 business day | 3 business days |
...
**Severity classification:** initial classification of an incident's priority is made by FarmGPU. If Customer disagrees with a classification, Customer may escalate through the support escalation path, and the parties will confer in good faith to resolve the classification promptly.
...
Enhanced credits are cumulative with availability credits, subject to the Service Credit Caps below.

## Service Credit Calculation; Caps
...
- Maximum Service Credits in any single calendar month: **50%** of the total monthly fees for the affected Order Form (equal to the highest single credit tier; enhanced credits are cumulative only up to this cap)
...
If, for Customer's applicable service tier:

(a) Monthly Uptime Percentage falls below the **Committed Monthly Uptime Percentage** for that tier for two (2) consecutive calendar months; OR

(b) Monthly Uptime Percentage falls more than **0.5 percentage points below** the Committed Monthly Uptime Percentage for that tier in any single calendar month (e.g., below 98.5% on the Standard tier, below 99.2% on the Premium tier, below 99.4% on the Enterprise tier);
